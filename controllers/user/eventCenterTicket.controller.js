    ...
    try {
        ...
        if (!ticket) {
            ...
            if (!ticket.ticketNumber) {
                ...
                ticket.qrPayload = generateQRPayload(ticket.ticketNumber, ticket.eventCenter);
            }
        }
    ...
    try {
        ...
        if (qrPayload) {
            ...
            if (!valid) {
                return res.status(400).json({
                    success: false,
                    message: "Invalid or tampered QR code. This booking pass may be forged.",
                });
            }
        ...
        if (!ticket) {
            ...
            const existingTicket = await EventCenterTicket.findOne({
                ticketNumber: lookupTicketNumber,
            }).select("status checkIn guestDetails").lean();
